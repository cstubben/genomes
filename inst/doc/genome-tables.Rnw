...
Finally, since the format of online genome tables may change (and then \Rcmd{update} commands may fail), I would recommend downloading
the development version for fixes in between the six month release cycle.
