...
metadata and provides tools to summarize, compare and plot the data in
...
(\url{http://www.ncbi.nlm.nih.gov}) include prokaryotic projects (\Rcmd{lproks}),
eukaryotic projects (\Rcmd{leuks}), and metagenomic projects
(\Rcmd{lenvs}).  The \Rcmd{print} methods displays the first few rows
...
method displays a cumulative plot of genomes by release date in Figure
...
run Entrez queries.  For example, users can retreive genome summaries
or neighbors using a valid Entrez search query, list taxonomy names
matching taxonomy ids, find the published dates of a pubmed ids, or
return the release dates for an accession number. The full details
