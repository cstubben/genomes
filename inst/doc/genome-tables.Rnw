...
There are a number of ways to install this package.  If you are running
\R~version 2.11, you can use the \Rcmd{biocLite} command.
...
You can also install the package on earlier versions of \R~using
\Rcmd{install.packages}, but this has not been tested completely.
...
Finally, since the format of online genome tables may change (and then
...
On some systems (Mac 10.4), you may need to add the
\Rcmd{type='source'} option to install the package source.  In
addition, the \Rcmd{genomes} package depends on some functions in the
\Rcmd{lattice} and \Rcmd{XML} packages, so these two should be
installed first (\Rcmd{lattice} is usually installed by default, but
not \Rcmd{XML}).
